...
def draw_shape(canvas, shape, x, y, size, label):
    if shape == "Triangle":
...
def play_game_gui():
    ...
    def start_game():
        ...
        canvas.delete("all")
        draw_shape(
            canvas, elements[player_choice]["shape"], 100, 300, 50, "Player"
        )  # Draw player shape with label
        draw_shape(
            canvas, elements[computer_choice]["shape"], 400, 300, 50, "Computer"
        )  # Draw computer shape with label
